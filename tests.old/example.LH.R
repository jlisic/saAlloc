...
x_adj[,1] <- v[1] * mean(z[,1])^gamm
x_adj[,2] <- v[2] * sqrt(sum(colMeans(z)^2))^gamm 
x_adj[,3] <- v[3] * sqrt(sum(colMeans(z)^2))^gamm 
x_adj[,4] <- v[4] * sqrt(sum(colMeans(z)^2))^gamm 
x_adj[,5] <- v[5] * mean(z[,2])^gamm 


# note, we do not generate or use Y since we already know the ACV.
